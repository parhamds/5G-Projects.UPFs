...
docker-build:
	for target in $(DOCKER_TARGETS); do \
		...
		if [ $(DOCKER_BUILDKIT) = 1 ]; then \
			DOCKER_CACHE_ARG="--cache-from ${DOCKER_REGISTRY}${DOCKER_REPOSITORY}upfs-$$target:${DOCKER_TAG}"; \
		fi; \
		DOCKER_BUILDKIT=$(DOCKER_BUILDKIT) docker build $(DOCKER_PULL) $(DOCKER_BUILD_ARGS) \
			--target $$target \
			$$DOCKER_CACHE_ARG \
			--tag ${DOCKER_REGISTRY}${DOCKER_REPOSITORY}upfs-$$target:${DOCKER_TAG} \
			--label org.opencontainers.image.source="https://github.com/parhamds/upfs" \
...
docker-push:
	for target in $(DOCKER_TARGETS); do \
		docker push ${DOCKER_REGISTRY}${DOCKER_REPOSITORY}upfs-$$target:${DOCKER_TAG}; \
	done
...
test: .coverage
	docker run --rm -v $(CURDIR):/upfs -w /upfs golang:latest \
		go test \
			-race \
			-failfast \
...
check-reuse:
	@docker run --rm -v $(CURDIR):/upfs -w /upfs omecproject/reuse-verify:latest reuse lint
